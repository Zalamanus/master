...
class CurrentUserFragment : Fragment(R.layout.fragment_current_user) {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        GithubViewModel.currentFollowingUsers.observe(viewLifecycleOwner) { followingUsers ->
            setFollowingUsersToUI(followingUsers)
        }
    ...
    private fun setFollowingUsersToUI(followingUsers: List<GitHubUser>) {
        if (followingUsers.isNotEmpty()) {
            followersTV.isVisible = true
            followersTitle.isVisible = true
            followersTV.text = followingUsers.joinToString(", ") {
                it.login
            }
        } else {
            followersTV.isVisible = false
            followersTitle.isVisible = false
        }
    }
